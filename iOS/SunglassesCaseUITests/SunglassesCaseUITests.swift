...
final class SunglassesCaseUITests: XCTestCase {
    ...
    func testAddItemFlow() {
        ...
        nameField.tap()
        nameField.typeText("UI Test Pair")
        app.buttons["itemSaveButton"].tap()
        XCTAssertTrue(app.staticTexts["UI Test Pair"].waitForExistence(timeout: 2))
    }
    ...
    func testFreeLimitTriggersPaywall() {
        for i in 0..<10 {
            ...
            app.buttons["addItemButton"].tap()
            if app.staticTexts["SunglassesCase Pro"].waitForExistence(timeout: 1) {
                break
            }
            let nameField = app.textFields["itemNameField"]
